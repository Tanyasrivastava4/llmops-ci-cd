...
def retrieve(query, docs):
    """Retrieve the most semantically relevant document for the query."""
    query_emb = retriever.encode(query, convert_to_tensor=True)
    doc_embs = retriever.encode(docs["text"].tolist(), convert_to_tensor=True)
    scores = util.pytorch_cos_sim(query_emb, doc_embs)
    best_idx = scores.argmax().item()
    return docs.iloc[best_idx]["text"]
...
def run_pipeline():
    ...
    for _, row in golden_df.iterrows():
        ...
        # Retrieve and generate
        context = retrieve(question, docs_df)
        predicted_answer, latency = generate_answer(model, tokenizer, context, question)

        # Track accuracy (case-insensitive)
        if predicted_answer.strip().lower() == golden_answer.strip().lower():
            correct += 1
        total_latency += latency
